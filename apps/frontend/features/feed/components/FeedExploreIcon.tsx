import type { ExploreItem } from "./feed-data"

export function FeedExploreIcon({ item }: { item: ExploreItem }) {
  switch (item.id) {
    case "learning":
      return (
        <svg
          ...
          fill="none"
          viewBox="0 0 20 20"
          className="mr-3.5 shrink-0 text-[var(--feed-explore-icon)]"
          aria-hidden
        >
          <path
            fill="currentColor"
    ...
    default:
      return (
        <span className="mr-3.5 inline-block size-5 shrink-0 rounded bg-[var(--feed-surface-input)]" />
      )
  }
}
